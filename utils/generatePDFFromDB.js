...
function formatDateTime(date) {
  if (!date) return "-";
  const d = new Date(date); d.setHours(d.getHours() + 8);
...
function wrapText(text, maxLength=80){
  ...
  words.forEach(w=>{ if((current+w).length>maxLength){ lines.push(current); current=w+" "; }else{ current+=w+" "; } });
...
export async function generatePDFWithLogo(requestId) {
  ...
  // ===== LOAD LOGO =====
  let logoImage=null;
  try { const logoBytes=fs.readFileSync("./assets/logo.png"); logoImage = await pdf.embedPng(logoBytes); } catch(e){ console.log("Logo tak jumpa bossskurrr"); }

  // ===== HEADER =====
  if(logoImage){
    const dims = logoImage.scaleToFit(60,60);
    page.drawImage(logoImage,{x:margin,y:y-20,width:dims.width,height:dims.height});
  }
  ...
  if(request.technicianRemark?.trim()){
    page.drawText("Catatan Technician:",{x:margin,y,size:11,font:bold}); y-=15;
    wrapText(request.technicianRemark).forEach(l=>{page.drawText(l,{x:margin+10,y,size:11,font}); y-=14;}); y-=10;
  }
  ...
  if(request.proofImageUrl){
    ...
    }catch(err){ console.log("Gagal load proof image"); }
  }
